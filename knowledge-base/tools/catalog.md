...
| 보류 | 현재 cross-platform core 우선순위 또는 성숙도·운영 경계상 도입하지 않음 |
| 역사 | 유지보수 종료 등으로 현재 제품 후보가 아닌 계보 근거로만 보존 |

이 카탈로그의 기존 등급은 조사 당시 축을 합치지 않고 `I2 / V2 / W0~W1`처럼 기록했다. 여기서 `V2`는 정적 분석 상한이며 모든 항목의 `V3+`는 미실행이다. `W0`은 Windows 관련 주장·간접 경로만 있는 상태, `W1`은 Windows 전용 코드나 설정 경로를 정적으로 확인한 상태다. 어느 쪽도 Windows 실행 성공을 뜻하지 않으며 Linux 등급으로 복제하지 않는다. 신규·갱신 Claim은 [스키마](../knowledge-graph-schema.md#플랫폼별-증거-p0p3)의 OS별 `P0~P3`을 사용한다.
