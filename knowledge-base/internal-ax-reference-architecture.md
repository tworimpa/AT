---
id: internal-ax-reference-architecture-v0
type: reference-architecture
title: Windows-first 사내 AX 에이전트 플랫폼 reference architecture
status: proposed
tags:
  - knowledge-base
  - ax-platform
...
# Windows-first 사내 AX 에이전트 플랫폼 reference architecture

[지식 베이스 홈](./index.md) · [지속 컨텍스트](./ax-platform-context.md) · [플랫폼 구현 청사진](./platform-blueprint.md) · [도구 카탈로그](./tools/catalog.md) · [프로필 커버리지](./tools/coverage.md) · [지식 그래프 스키마](./knowledge-graph-schema.md)
...
3. Windows local executor는 ConPTY, Job Object, process-tree 회수, long path와 CRLF를 first-class 계약으로 둔다. `W2/W3` 전까지 구현 완료로 주장하지 않는다.
...
6. local, WSL, remote Linux, container와 cloud sandbox가 같은 인터페이스를 구현해도 isolation·egress·resume semantics의 차이는 capability profile에 드러낸다.
...
| Executor | Windows process/PTY/job lifecycle, worktree와 non-file resource lease, environment fingerprint, local/remote provider contract | worktree를 security sandbox로 과장하지 않음; remote Linux를 native Windows로 세지 않음 | Orca·Emdash·Container Use·E2B·Vercel·Cloudflare 조사 (`V1/V2`, Windows 최대 `W1`) |
...
| Windows local executor | ConPTY, Job Object, process-tree cancel, staged worktree, port/db/device resource lease | 실제 Windows `W2`, orphan process 0 |
...
|---|---|---|
| WSL/SSH/container/cloud executor | data class·egress·secret·residency 결정과 provider conformance | 동일 executor contract의 capability-limited 구현 |
...
아래 관계는 설계 후보이며 승인된 회사 결정이 아니다. `Source`는 현재 최대 `I2/V2/W0~W1` 정적 근거다.
...
| Windows process containment | 직원 PC에서 취소·crash 후 process 회수 | `AD-PROP-003`: ConPTY + Job Object native executor | `RM-P1-windows-executor`, `RM-P5-windows-regression` | Orca/Codex 등 Windows 정적 경로 최대 `W1`; `W2/W3` 없음 |
...
| Adapt | 회사 조건 또는 Windows executor에 맞춰 의미를 좁히고 검증 계획을 붙임 | tmux/WSL orchestration을 ConPTY/Job Object lifecycle로 변형 |
...
| evidence grade | artifact 없는 승격 거부 | `I2`·`W1`·CI 존재를 `V3+` 또는 production으로 혼합 |
...
[플랫폼 청사진](./platform-blueprint.md)의 P0~P5는 기술 구현 순서 후보다. 회사 decision-needed 항목이 답을 얻기 전에는 production roadmap 또는 일정 약속이 아니다. 첫 허용 실행은 Windows local kernel의 좁은 `V3/W2` pilot이며, remote executor·external write·실제 credential·비용 발생 서비스는 별도 profile과 승인 후 진행한다.
