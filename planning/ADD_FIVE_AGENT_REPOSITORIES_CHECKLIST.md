...
- [ ] 5개 저장소가 `.gitmodules`와 mode `160000` gitlink에 등록돼 있다.
- [ ] 각 gitlink는 조사 시점의 실제 기본 브랜치 HEAD를 가리킨다.
- [ ] 라이선스 gate를 통과한 저장소만 분석 대상에 포함한다.
- [ ] 조사 대상 수와 분류, 근거 permalink가 모든 관련 문서에서 일치한다.
- [ ] 확인한 정적 사실과 실행하지 않은 검증을 명확히 구분한다.
- [ ] 검증 명령이 통과하고 변경사항이 하나의 재현 가능한 PR로 제출된다.

## 1. 사전 확인

- [ ] 작업 트리가 clean인지 확인한다: `git status --short --branch`.
- [ ] 저장소 루트와 상위 경로를 포함해 적용되는 모든 `AGENTS.md`를 읽는다.
- [ ] 다음 파일의 현재 구조, 표 형식과 증거 작성 방식을 다시 확인한다.
  - [ ] `README.md`
  - [ ] `.gitmodules`
...
- [ ] 기존 28개 submodule의 변경이나 dirty 상태가 없는지 확인한다.

## 2. Submodule 추가

각 저장소에 대해 기본 브랜치를 원격에서 확인한 후 `git submodule add --depth 1 <URL> <PATH>`로 추가한다.

- [ ] DeepSeek Harness를 `multi-agent-tools/deepseek-harness`에 추가한다.
- [ ] Hermes Agent를 `multi-agent-tools/hermes-agent`에 추가한다.
- [ ] OpenClaw를 `multi-agent-tools/openclaw`에 추가한다.
- [ ] OpenAI Codex를 `multi-agent-tools/codex`에 추가한다.
- [ ] Cline을 `multi-agent-tools/cline`에 추가한다.
- [ ] 각 저장소의 기본 브랜치 이름과 `git rev-parse HEAD` 결과를 기록한다.
- [ ] 부모 저장소의 gitlink가 해당 SHA와 정확히 일치하는지 확인한다.
- [ ] `.gitmodules`의 URL과 경로가 위 표의 공식 값과 일치하는지 확인한다.
- [ ] 임의 SHA, branch 이름 추측 또는 존재하지 않는 placeholder gitlink를 사용하지 않는다.

## 3. 라이선스 gate

각 저장소는 상세 분석 전에 다음 순서로 확인한다.

- [ ] 루트 `LICENSE`, `LICENSE.*`, `NOTICE`, README의 license 절을 확인한다.
- [ ] source-available 조건, 사용 분야 제한, 분석·벤치마크 제한 등 추가 rider를 검색한다.
- [ ] 하위 패키지와 vendored component에 별도 라이선스가 있는지 확인한다.
- [ ] 사용 또는 분석을 제한하는 rider가 있으면 해당 저장소의 분석과 submodule 등록을 중단하고 근거만 기록한다.
- [ ] MIT 또는 Apache-2.0이고 제한 rider가 없을 때만 분석을 계속한다.
- [ ] SPDX 이름만 신뢰하지 않고 고정 checkout의 실제 license text와 notice를 근거로 삼는다.
...
- [ ] “28개 저장소 코드·GitHub 분석”을 “33개 저장소 코드·GitHub 분석”으로 변경한다.

### `multi-agent-tools/README.md`

- [ ] 조사 대상 수를 33개로 변경한다.
- [ ] 빠른 분류 표에 5개 프로젝트를 추가한다.
- [ ] Windows 검토 순서에 5개 프로젝트를 추가한다.
- [ ] 정확한 클론 기준점 표에 실제 기본 브랜치와 checkout SHA를 추가한다.
- [ ] 각 프로젝트의 라이선스와 실행 미검증 경계를 표시한다.

### `planning/REPOSITORY_GITHUB_ANALYSIS.md`

- [ ] 분석 대상을 33개로 변경한다.
- [ ] coding-agent runtime형에 Codex와 Cline을 추가한다.
- [ ] personal assistant runtime·gateway형에 Hermes Agent와 OpenClaw를 추가한다.
- [ ] plugin-composed agent harness형에 DeepSeek Harness를 추가한다.
- [ ] 각 저장소의 강점, 한계와 고정 SHA permalink를 추가한다.
...
- [ ] 라이선스 표를 고정 checkout 근거로 갱신한다.

### `planning/AI_CODING_AGENT_TOOLS_AND_SERVICES_LANDSCAPE.md`

- [ ] 조사 대상을 33개로 변경한다.
- [ ] 5개 프로젝트를 제품 계층별로 추가한다.
- [ ] 확인한 사실과 실행 미검증 사항을 별도 열 또는 문장으로 구분한다.

### `planning/FAST_MULTI_AGENT_ORCHESTRATION_PLAN.md`

- [ ] 근거 문서를 33개 저장소 분석으로 변경한다.
- [ ] Codex app-server/exec/TUI를 서로 다른 capability profile로 취급한다.
- [ ] Cline IDE/CLI/hub surface를 서로 다른 capability profile로 취급한다.
- [ ] Hermes/OpenClaw channel request를 task intake로 바꾸는 Assistant Gateway Connector를 추가한다.
- [ ] sender identity, workspace, credential audience, sandbox profile을 원자적으로 binding한다.
- [ ] 불완전하거나 만료된 binding을 fail-closed하도록 명시한다.

### `planning/AI_AGENT_DEVELOPMENT_ENVIRONMENT_REQUIREMENTS.md`

- [ ] 실행 surface별 capability snapshot과 protocol version 추적 요구사항을 추가한다.
- [ ] gateway channel provenance와 durable intake event 요구사항을 추가한다.
- [ ] sender identity, workspace, credential audience, sandbox profile binding 요구사항을 추가한다.
- [ ] memory, skill, cron 및 companion node에서 유래한 요청의 provenance 요구사항을 추가한다.
- [ ] assistant gateway와 coding workspace의 trust domain 분리 인수 조건을 추가한다.

## 6. 검증

- [ ] `git submodule status`에 기존 28개와 신규 5개가 모두 나타나는지 확인한다.
- [ ] 깨끗한 별도 clone에서 `git submodule update --init --recursive --depth 1`을 실행한다.
- [ ] 5개 checkout의 `git rev-parse HEAD`와 부모 gitlink를 비교한다.
- [ ] 5개 checkout의 현재 branch 또는 detached HEAD와 기록한 기본 브랜치를 구분한다.
- [ ] 문서에서 오래된 `28개` 표현을 검색하고 의도된 역사적 문맥 외에는 제거한다.
- [ ] 모든 GitHub source link가 고정 checkout SHA permalink인지 확인한다.
- [ ] 라이선스 표와 각 저장소의 실제 license text가 일치하는지 확인한다.
- [ ] `git diff --check`를 실행한다.
- [ ] 변경된 Markdown의 표와 링크를 검사한다.
- [ ] 의존성 설치·build·Windows·remote provider·실 agent E2E를 실행하지 않았다면 명시적으로 미검증으로 남긴다.
...
2026-08-14의 최초 시도에서는 GitHub HTTPS 연결이 `CONNECT tunnel failed, response 403`으로 차단됐고 GitHub CLI 인증도 제공되지 않았다. 그 상태에서는 upstream 기본 브랜치, checkout SHA, license rider와 source permalink를 검증할 수 없으므로 submodule이나 분석 문서를 임의 값으로 만들지 않았다. 후속 작업자는 GitHub 접근이 복구된 환경에서 이 목록을 처음부터 수행해야 한다.
